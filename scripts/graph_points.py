...
class points:
    def __init__(self):
        ...
        linear = pd.read_csv(ruta + '/datosr.csv')
        x_linear = [float(i.replace(',', '.')) - 0.1 for i in linear["error_x"]]
        y_linear = [float(i.replace(',', '.')) for i in linear["error_y"]]
        angulares = pd.read_csv(ruta + '/angulos.csv')
        theta_exp = angulares["error_angulos"]
        theta_exp = theta_exp
        ...
        for i in range (40):
            ...
            theta = float(theta_exp[i].replace(',', '.'))
            quat = quaternion_from_euler(0.0 , 0.0 ,theta)
